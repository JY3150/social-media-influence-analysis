...
draw_graphs <- function(model_tbl_fitted, reverse, result) {
  if(!reverse) {
    ...
    ggsave(file=glue('{result}/actual_ma.png'), plot=plot1)
    ggsave(file=glue('{result}/prediction_ma.png'), plot=plot2)
    ...
    plot2 <- ggplot(model_tbl_fitted, aes(x = time_window, y = fitted)) +
      ...
      labs(title = "Predicted Demand vs Time Window",
           ...
           y = "Prediction") +
      theme_minimal()
    ggsave(file=glue('{result}/actual_reversed.png'), plot=plot1)
    ggsave(file=glue('{result}/prediction_reversed.png'), plot=plot2)
  }
}
